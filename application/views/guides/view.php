        ...
        <div class="card-content">
            ...
            <div id="invoice-template" class="card-body">
                <div class="row">
                    ...
                    <div class="">
                        <?php
							$validtoken = hash_hmac('ripemd160', $guide['iid'], $this->config->item('encryption_key'));
							$link = base_url('guides/view?id=' . $guide['iid'] . '&token=' . $validtoken);
                        ?>
                          <div class="title-action">
							<?php 
							if($guide['status'] == 0)
							{
								echo '<a href="edit?id=' . $guide['iid'].'" class="btn btn-warning mb-1" ';
								if ($this->aauth->get_user()->roleid < 5) echo 'disabled';
								echo '><i class="fa fa-pencil" ></i> Alterar '.$guide['type'].'</a>';
							}
							else if($guide['status'] == 1)
							{
								echo '<h2 class="btn btn-oval btn-danger">Finalizado o Documento</h2>';
							}else{
								echo '<h2 class="btn btn-oval btn-danger">Anulado</h2>';
							}?>
                            ...
                            <div class="btn-group ">
                                ...
                                <div class="dropdown-menu">
                                    <a class="dropdown-item"
                                       href="<?php echo base_url('guides/printguide?id=' . $guide['iid'] . '&token=' . $validtoken); ?>"><?php echo $this->lang->line('Print') ?></a>
                                    <div class="dropdown-divider"></div>
                                    <a class="dropdown-item"
                                       href="<?php echo base_url('guides/printguide?id=' . $guide['iid'] . '&token=' . $validtoken); ?>&d=1"><?php echo $this->lang->line('PDF Download') ?></a>
				...
				<ul class="nav nav-tabs" role="tablist">
					<li class="nav-item">
						<a class="nav-link active show" id="base-tab1" data-toggle="tab"
						   ...
						   aria-selected="true">Detalhes <?php echo $guide['type']; ?> <strong class="pb-1"> Nº<?php echo $guide['irs_type_s'].' '.$guide['serie_name'] . '/' . $guide['tid']; ?></strong></a>
					</li>
					<li class="nav-item">
						...
						<div class="table-responsive col-sm-12">
							<table class="table table-striped">
								<thead>
									...
									<tbody>
									<tr>
										<td><?php echo $guide['ref_enc_orc']; ?></td>
										<td><?php echo $guide['refer']; ?></td>
										<td><?php echo $guide['multiname']; ?></td>
									</tr>
								</tbody>
							</table>
						</div>
						...
						<div id="invoice-customer-details" class="row pt-2">
							<div class="col-md-6 col-sm-12 text-xs-center text-md-left">
								<ul class="px-0 list-unstyled">
									<li class="text-bold-800"><li>Código: <?php echo $guide['csd']?></li><a href="<?php echo base_url('customers/view?id=' . $guide['cid']) ?>"><strong
											   class="invoice_a"><?php echo $guide['name'] . '</strong></a></li><li>' . $guide['company'] . '</li><li>' . $guide['address'] . '</li><li>' . $guide['city'] . ',' . $guide['country'] . '</li><li>' . $this->lang->line('Phone') . ': ' . $guide['phone'] . '</li><li>' . $this->lang->line('Email') . ': ' . $guide['email'] . '</li>';
										if (CUSTOM) {
							...
							<div class="offset-md-3 col-md-3 col-sm-12 text-xs-center text-md-left">
								<?php echo '<p><span class="text-muted">Data da Emissão  :</span> ' . dateformat($guide['invoicedate']) . '</p> <p><span class="text-muted">' . $this->lang->line('Due Date') . ' :</span> ' . dateformat($guide['invoiceduedate']) . '</p>  <p><span class="text-muted">' . $this->lang->line('Terms') . ' :</span> ' . $guide['termtit'] . '</p><p><span class="text-muted">Série :</span> ' . $guide['irs_type_s'].' '. $guide['serie_name'] . '</p>';
								?>
							</div>
						</div>
						<!--/ Invoice Customer Details -->
												...
												foreach ($products as $row) {
													...
													echo '<tr>
														<th scope="row">' . $c . '</th>
															...
															<td>' . amountExchange($row['price'], $guide['multi'], $guide['loc']) . '</td>
															 <td>' . amountFormat_general($row['qty']) .' '. $row['unit'] . '</td>
															 <td>' . amountExchange($row['totaldiscount'], $guide['multi'], $guide['loc']) . ' (' . amountFormat_s($row['discount']) . $this->lang->line($guide['format_discount']) . ')</td>
															<td>' . amountExchange($valsum, $guide['multi'], $guide['loc']) . ' (' . $row['taxaperc'] . '%)</td>
															<td>' . amountExchange($row['totaltax'], $guide['multi'], $guide['loc']) . '</td>
														</tr>';
									...
									<div class="table-responsive">
										<table class="table">
											...
											<tr>
												...
												<td class="text-xs-right"><?php echo amountExchange($guide['discount'], $guide['multi'], $guide['loc']) ?></td>
											</tr>
											...
											<tr>
												...
												<td class="text-xs-right"><?php echo amountExchange($guide['shipping'], 0, $this->aauth->get_user()->loc) ?></td>
											</tr>
											<tr>
												<td><?php echo $this->lang->line('Discount') ?> Financeiro</td>
												<td class="text-xs-right"><?php echo amountExchange($guide['discount_rate'], $guide['multi'], $guide['loc']) ?></td>
											</tr>
											<tr>
												<td class="text-bold-800"><?php echo $this->lang->line('Total') ?></td>
												<td class="text-bold-800 text-xs-right"> <?php echo amountExchange($guide['total'], 0, $this->aauth->get_user()->loc) ?></td>
											</tr>
											</tbody>
										</table>
									</div>
						...
						<div class="row">
							<table class="table table-striped">
								...
								<?php foreach ($attach as $row) {
									...
									echo '<tr><td><a data-url="' . base_url() . 'guides/file_handling?op=delete&name=' . $row['col1'] . '&invoice=' . $guide['iid'] . '" class="aj_delete"><i class="btn-danger btn-lg fa fa-trash"></i></a> <a class="n_item" href="' . base_url() . 'userfiles/attach/' . $row['col1'] . '"> ' . $row['col1'] . ' </a></td></tr>';
								} ?>
...
<script>
    ...
    $(function () {
        ...
        var url = '<?php echo base_url() ?>invoices/file_handling?id=<?php echo $guide['iid'] ?>';
        $('#fileupload').fileupload({
            ...
            formData: {'<?php echo$this->security->get_csrf_token_name()?>': crsf_hash},
                ...
                $.each(data.result.files, function (index, file) {
                    $('#files').append('<tr><td><a data-url="<?php echo base_url() ?>invoices/file_handling?op=delete&name=' + file.name + '&invoice=<?php echo $guide['iid'] ?>" class="aj_delete red"><i class="btn-sm fa fa-trash"></i></a> ' + file.name + ' </td></tr>');
                });
                    ...
                    <div class="row">
                        <div class="col mb-1"><label
                            ...
                            <input type="text" class="form-control"
                                   ...
                                   value="Pagamento da Factura: #<?php echo $guide['irs_type'] . ' '.$guide['serie'].'/'.$guide['tid']; ?>"></div>
                    </div>
                    <div class="modal-footer">
                        <input type="hidden" class="form-control required"
                               name="tid" id="invoiceid" value="<?php echo $guide['iid'] ?>">
                        <button type="button" class="btn btn-default"
                                data-dismiss="modal"><?php echo $this->lang->line('Close') ?></button>
                        <input type="hidden" name="cid" value="<?php echo $guide['cid'] ?>"><input type="hidden"
                                                                                                     name="cname"
                                                                                                     value="<?php echo $guide['name'] ?>">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <input type="hidden" class="form-control"
                       name="tid" value="<?php echo $guide['iid'] ?>">
				<input type="hidden" class="form-control"
                       name="tid" value="<?php echo $guide['iid'] ?>">
                <button type="button" class="btn btn-default"
                        data-dismiss="modal"><?php echo $this->lang->line('Close'); ?></button>
                ...
                <form id="sendbill">
                    ...
                    <div class="row">
                        <div class="col mb-1"><label
                            ...
                            <input type="text" class="form-control"
                                   name="customername" value="<?php echo $guide['name'] ?>"></div>
                    </div>
                    <div class="row">
                        <div class="col mb-1"><label
                    ...
                    <input type="hidden" class="form-control"
                           id="invoiceid" name="tid" value="<?php echo $guide['iid'] ?>">
                    <input type="hidden" class="form-control"
