						 ...
						 <div class="btn-group ">
							...
							<div class="dropdown-menu">
								<a class="dropdown-item"
								   href="<?php echo base_url('customers_notes/printinvoice?id=' . $invoice['iid'] . '&ty=1&token=' . $validtoken); ?>"><?php echo $this->lang->line('Print') ?></a>
								<div class="dropdown-divider"></div>
								<a class="dropdown-item"
								   href="<?php echo base_url('customers_notes/printinvoice?id=' . $invoice['iid'] . '&ty=1&token=' . $validtoken); ?>&d=1"><?php echo $this->lang->line('PDF Download') ?></a>
						...
						<div class="row">
							<table class="table table-striped">
								...
								<?php foreach ($attach as $row) {
									...
									echo '<tr><td><a data-url="' . base_url() . 'customers_notes/file_handling?op=delete&name=' . $row['col1'] . '&invoice=' . $invoice['iid'] . '" class="aj_delete"><i class="btn-danger btn-lg fa fa-trash"></i></a> <a class="n_item" href="' . base_url() . 'userfiles/attach/' . $row['col1'] . '"> ' . $row['col1'] . ' </a></td></tr>';
								} ?>
...
<script>
    ...
    $(function () {
        ...
        var url = '<?php echo base_url() ?>customers_notes/file_handling?id=<?php echo $invoice['iid'] ?>';
        $('#fileupload').fileupload({
            ...
            formData: {'<?php echo$this->security->get_csrf_token_name()?>': crsf_hash},
                ...
                $.each(data.result.files, function (index, file) {
                    $('#files').append('<tr><td><a data-url="<?php echo base_url() ?>customers_notes/file_handling?op=delete&name=' + file.name + '&invoice=<?php echo $invoice['iid'] ?>" class="aj_delete red"><i class="btn-sm fa fa-trash"></i></a> ' + file.name + ' </td></tr>');
                });
