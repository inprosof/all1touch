...
<head>
    ...
    <title>Extrato da Conta Cliente</title>
...
<div class="invoice-box">
    <table>
        <tr>
            ...
            <td class="myw">
                <?php 
					echo $this->lang->line('Account Statement');
					if($customer != null)
					{
						if($customer['gr_client'] != null)
						{
							echo '<br>Grupo: '.$customer['gr_client'];
						}
					}
				?>
            </td>
    ...
    <table class="plist" cellpadding="0" cellspacing="0">
        ...
        foreach ($list as $row) {
            ...
            $balanceline += $row['credit'] - $row['debit'];
            echo '<tr class="item' . $flag . '"><td>' . $row['date'] . '</td><td>' . $row['cod_cat'] . '</td><td>' . $row['name_cat'];
			if($row['note'] != null && $row['note'] != "")
			{
				echo '('.$row['note'].')';
			}
			echo '</td><td>' . amountExchange($row['debit'], 0, $this->aauth->get_user()->loc) . '</td><td>' . amountExchange($row['credit'], 0, $this->aauth->get_user()->loc) . '</td><td>' . amountExchange($balanceline, 0, $this->aauth->get_user()->loc) . '</td></tr>';
            $fill = !$fill;
        }
