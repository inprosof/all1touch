...
class Quote_model extends CI_Model
{
    ...
    public function quote_details($id)
    {
		...
		{
			$this->db->select("geopos_quotes.*, geopos_quotes.id as iddoc, CASE WHEN geopos_locations.address = '' OR geopos_locations.address IS NULL THEN geopos_system.address ELSE geopos_locations.cname END AS loc_name
			...
			$this->db->select("geopos_quotes.*, geopos_quotes.id as iddoc, CASE WHEN geopos_locations.cname = '' THEN geopos_system.cname ELSE geopos_locations.cname END AS loc_name
		...
		$this->db->join('geopos_assets', 'geopos_quotes.autoid = geopos_assets.id', 'left');
		$this->db->join('geopos_countrys as l1', 'geopos_quotes.charge_country = l1.prefix', 'left');
		$this->db->join('geopos_countrys as l2', 'geopos_quotes.discharge_country = l2.prefix', 'left');
		...
		$this->db->join('geopos_config as c1', 'c1.id = geopos_quotes.expedition', 'left');
    ...
    {
        $this->db->select('geopos_quotes.id,geopos_series.serie AS serie_name,geopos_quotes.tid,geopos_quotes.invoicedate,geopos_quotes.invoiceduedate,geopos_quotes.total,geopos_quotes.status,geopos_customers.name');
        $this->db->from($this->table);
