...
class Categories_model extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT id,title,cod FROM geopos_product_cat WHERE c_type='$type' AND rel_id='$rel' ORDER BY id DESC");
...
ORDER BY id DESC");
    ...
    public function cat_ware($id, $loc = 0)
    {
        ...
        $query = $this->db->query("SELECT c.id AS cid, w.id AS wid,c.title AS catt,w.title AS watt,uu.id as claid, uu.title as clatit FROM geopos_products AS p LEFT JOIN geopos_product_cat AS c ON p.pcat=c.id LEFT JOIN geopos_warehouse AS w ON p.warehouse=w.id LEFT JOIN geopos_products_class AS uu ON p.p_cla=uu.id WHERE
p.pid='$id' $qj ");
        return $query->row_array();
    }


    public function addnew($cat_name, $cat_desc, $cat_type = 0, $cat_rel = 0, $cat_cod = '')
    {
        if (!$cat_type) $cat_type = 0;
        if (!$cat_rel) $cat_rel = 0;
		if (!$cat_cod) $cat_cod = 'P';
        $data = array(
			'cod' => $cat_cod,
            'title' => $cat_name,
            'extra' => $cat_desc,
    ...
    public function edit($catid, $product_cat_name, $product_cat_desc, $cat_type, $cat_rel, $old_cat_type, $cat_cod)
    {
         if (!$cat_rel) $cat_rel = 0;
        $data = array(
			'cod' => $cat_cod,
