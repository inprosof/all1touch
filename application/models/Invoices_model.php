...
class Invoices_model extends CI_Model
{
    ...
    public function invoice_details($id, $eid = '',$p=true)
    {
		...
		{
			$this->db->select("geopos_invoices.*, geopos_invoices.id as iddoc, 
			CASE WHEN geopos_locations.address = '' OR geopos_locations.address IS NULL THEN geopos_system.address ELSE geopos_locations.cname END AS loc_name
			, CASE WHEN geopos_locations.address = '' OR geopos_locations.address IS NULL THEN geopos_system.address ELSE geopos_locations.address END AS loc_adress
			, CASE WHEN geopos_locations.postbox = '' OR geopos_locations.postbox IS NULL THEN geopos_system.postbox ELSE geopos_locations.postbox END AS loc_postbox
			, CASE WHEN geopos_locations.city = '' OR geopos_locations.city IS NULL THEN geopos_system.city ELSE geopos_locations.city END AS loc_city
			, CASE WHEN geopos_locations.country = '' OR geopos_locations.country IS NULL THEN geopos_system.country ELSE geopos_locations.country END AS loc_country,
	...
	public function detailsFromNT($custid)
    {
        $this->db->select('geopos_purchase.*,geopos_customers.*');
	...
	public function items_with_product_note_enco($id)
    {
	...
	public function items_with_payments($id)
    {
        $this->db->select('geopos_draft_payments.*,geopos_config.val1 AS name_pay,geopos_config.val2 as datapay');
        $this->db->from('geopos_draft_payments');
        $this->db->where('geopos_draft_payments.tid', $id);
        $this->db->join('geopos_config', 'geopos_config.id = geopos_draft_payments.pay_met', 'left');
        $query = $this->db->get();
        return $query->result_array();
    ...
    public function warehouses()
    {
		...
		if ($name) {
			$this->db->where('UPPER(geopos_warehouse.title) LIKE', '%'.strtoupper($name).'%');
		}
        $query = $this->db->get();
