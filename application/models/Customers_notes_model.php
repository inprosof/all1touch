...
class Customers_notes_model extends CI_Model
{
	...
	var $column_search = array('geopos_series.serie AS serie_name', 'geopos_customers_notes.tid', 'geopos_customers.name', 'geopos_customers_notes.invoicedate', 'geopos_customers_notes.total', 'geopos_customers_notes.status', null);
	...
	public function custumers_notes_details2($id, $eid = '',$p=true)
    {
		...
		CASE WHEN geopos_locations.cname = '' THEN geopos_system.cname ELSE geopos_locations.cname END AS loc_cname,geopos_currencies.code as multiname, geopos_series.serie AS serie_name, geopos_irs_typ_doc.type AS irs_type_s, geopos_irs_typ_doc.description AS irs_type_n, SUM(geopos_draft.shipping + geopos_draft.ship_tax) AS shipping,geopos_customers.*,geopos_draft.loc as loc,geopos_draft.id AS iid,geopos_customers.id AS cid,geopos_terms.id AS termid,geopos_terms.title AS termtit,geopos_terms.terms AS terms");
    ...
    public function custumers_notes_details($id)
    {
		...
		{
			$this->db->select("geopos_customers_notes.*, geopos_customers_notes.id as iddoc, CASE WHEN geopos_locations.address = '' OR geopos_locations.address IS NULL THEN geopos_system.address ELSE geopos_locations.cname END AS loc_name
			...
			$this->db->select("geopos_customers_notes.*, geopos_customers_notes.id as iddoc, CASE WHEN geopos_locations.address = '' THEN geopos_system.address ELSE geopos_locations.address END AS loc_name
			, CASE WHEN geopos_locations.address = '' THEN geopos_system.address ELSE geopos_locations.address END AS loc_adress
			, CASE WHEN geopos_locations.postbox = '' THEN geopos_system.postbox ELSE geopos_locations.postbox END AS loc_postbox
			, CASE WHEN geopos_locations.city = '' THEN geopos_system.city ELSE geopos_locations.city END AS loc_city
			, CASE WHEN geopos_locations.country = '' THEN geopos_system.country ELSE geopos_locations.country END AS loc_country,
