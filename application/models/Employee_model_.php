...
class Employee_model extends CI_Model
{
    ...
    public function update_employee($id, $name, $phone, $phonealt, $address, $city, $region, $country, $postbox, $location, $basic_salary = 0,$house_allowance=0,$medical_allowance=0,$special_allowance=0, $other_allowance=0,$tax_deduct=0,$provident_deduct=0,$other_deduct=0, $total_salary=0, $department = -1, $commission = 0, $roleid = false)
    {
        $this->db->select('total_salary');
        $this->db->from('geopos_employees');
        $this->db->where('id', $id);
        $query = $this->db->get();
        $sal = $query->row_array();
        ...
        $data = array(
            ...
            'provident_deduct' => $provident_deduct,
        ...
        if ($this->db->update('geopos_employees')) {
            ...
            if (($salary != $sal['total_salary']) AND ($salary > 0.00)) {
                $data1 = array(
                    'typ' => 1,
                    'rid' => $id,
                    'val1' => $salary,
                    'val2' => $sal['total_salary'],
    ...
    public function add_employee($id, $username, $name, $roleid, $phone, $address, $city, $region, $country, $postbox, $location, $basic_salary = 0,$house_allowance=0,$medical_allowance=0,$special_allowance=0, $other_allowance=0,$tax_deduct=0,$provident_deduct=0,$other_deduct=0, $total_salary=0, $commission = 0, $department = 0)
    {
        $data = array(
            'id' => $id,
            'username' => $username,
