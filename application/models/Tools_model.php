...
class Tools_model extends CI_Model
{
    ...
    var $notecolumn_order = array(null, 'geopos_notes.cdate', 'geopos_notes.last_edit', 'geopos_notes.title', 'name_add', 'cli_add');
    var $notecolumn_search = array('geopos_notes.id', 'geopos_notes.cdate', 'geopos_notes.last_edit', 'geopos_notes.title', 'name_add', 'cli_add');
    ...
    public function viewtask($id)
    {
		...
		$this->db->order_by('geopos_todolist.status', 'DESC');
		$this->db->order_by('geopos_todolist.priority', 'ASC');
        $query = $this->db->get();
    ...
    public function task_stats()
    {
        $query = $this->db->query("SELECT
				COUNT(IF( status = 'Vencida', id, NULL)) AS Vencida,
				COUNT(IF( status = 'Progresso', id, NULL)) AS Progresso,
				COUNT(IF( status = 'Feita', id, NULL)) AS Feita
    ...
    private function _notes_datatables_query()
    {
		...
		$this->db->where('geopos_notes.cid', $this->aauth->get_user()->id);
        $i = 0;
    ...
    {
        $this->db->select('*,geopos_employees.name as employee');
        $this->db->from('geopos_notes');
		$this->db->join('geopos_users', 'geopos_users.id = geopos_notes.cid', 'left');
		$this->db->join('geopos_employees', 'geopos_employees.username = geopos_users.username', 'left');
        $this->db->where('geopos_notes.id', $id);
        $query = $this->db->get();
        return $query->row_array();
    }
    ...
    var $doccolumn_order = array(null, 'geopos_documents.cdate', 'geopos_documents.title', 'name_add', 'cli_add','geopos_documents.filename');
    var $doccolumn_search = array('geopos_documents.cdate', 'geopos_documents.title', 'name_add', 'cli_add');
    ...
    {
        $data = array('title' => $title, 'filename' => $filename, 'cdate' => date('Y-m-d'), 'cid' => $this->aauth->get_user()->id, 'fid' => 0, 'rid' => 0);
    ...
    {
		$this->db->select("geopos_documents.id, geopos_documents.cdate, geopos_documents.title, geopos_documents.filename, geopos_employees.name as name_add, 
		CASE WHEN geopos_customers.name = '' OR geopos_customers.name IS NULL THEN 'Doc. Empresa' ELSE geopos_customers.name END as cli_add,geopos_documents.filename");
        $this->db->from('geopos_documents');
		$this->db->join('geopos_customers', 'geopos_customers.id = geopos_documents.fid', 'left');
