...
class Supplier_notes_model extends CI_Model
{
	...
	var $column_search = array('geopos_series.serie', 'geopos_supplier_notes.tid', 'geopos_supplier.name', 'geopos_supplier_notes.invoicedate', 'geopos_supplier_notes.total', 'geopos_supplier_notes.status');
    ...
    public function supplier_notes_details($id)
    {
		...
		{
			$this->db->select("geopos_supplier_notes.*,
				geopos_data_transport.autoid,
				geopos_data_transport.expedition,
				geopos_data_transport.exp_date,
				geopos_data_transport.exp_mat,
				...
				geopos_data_transport.discharge_country, CASE WHEN geopos_locations.address = '' THEN geopos_system.address ELSE geopos_locations.address END AS loc_name,
				geopos_assets.assest_name as autoid_name,l1.name as charge_country_name, l2.name as discharge_country_name, c1.val2 as expd_t, c1.val1 as expd_name, 
				geopos_supplier_notes.id as iddoc, 
				...
				SUM(geopos_supplier_notes.shipping + geopos_supplier_notes.ship_tax) AS shipping,geopos_customers.*,
				copys.val2 as numcop,
				geopos_supplier_notes.loc as loc,geopos_supplier_notes.id AS iid,geopos_customers.id AS cid,geopos_terms.id AS termid,geopos_terms.title AS termtit,geopos_terms.terms AS terms");
		}else{
			$this->db->select("geopos_supplier_notes.*,
				geopos_data_transport.autoid,
				geopos_data_transport.expedition,
		...
		$this->db->join('geopos_data_transport', 'geopos_supplier_notes.id=geopos_data_transport.tid', 'left');
		$this->db->join('geopos_assets', 'geopos_data_transport.autoid = geopos_assets.id', 'left');
		$this->db->join('geopos_countrys as l1', 'geopos_data_transport.charge_country = l1.prefix', 'left');
		$this->db->join('geopos_countrys as l2', 'geopos_data_transport.discharge_country = l2.prefix', 'left');
		$this->db->join('geopos_config as c1', 'c1.id = geopos_data_transport.expedition', 'left');
		$this->db->join('geopos_accounts', "geopos_accounts.loc = geopos_supplier_notes.loc and geopos_accounts.payonline = 'Yes'", 'left');
		$this->db->join('geopos_system', 'geopos_system.id = 1', 'left');
		$this->db->join('geopos_locations', 'geopos_locations.id = geopos_supplier_notes.loc', 'left');
		$this->db->join('geopos_warehouse as w1', 'geopos_locations.ware=w1.id', 'left');
		$this->db->join('geopos_warehouse as w2', 'geopos_supplier_notes.loc=w2.id', 'left');
		...
		$this->db->join('geopos_documents_copys', 'geopos_supplier_notes.loc = geopos_documents_copys.loc and geopos_supplier_notes.irs_type = geopos_documents_copys.typ_doc', 'inner');
		$this->db->join('geopos_config as copys', 'geopos_documents_copys.copy = copys.id', 'left');
    ...
    var $column_search2 = array('geopos_series.serie', 'geopos_draft.tid', 'geopos_supplier.name', 'geopos_draft.invoicedate', 'geopos_draft.total');
