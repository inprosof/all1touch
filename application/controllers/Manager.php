...
class Manager extends CI_Controller
{
    ...
    public function todo_load_list()
    {
        ...
        foreach ($list as $task) {
            ...
            if ($task->status == 'Due') {
				...
				$name = '<a href="#" data-id="' . $task->id . '" class="view_task">' . $task->name . '</a>';
			}
    ...
    public function pendingtasks()
    {
        ...
        foreach ($tasks as $row) {
            $tlist .= '<div class="media">
                        <div class="media-left valign-middle"><i class="fa fa-bullhorn icon-bg-circle bg-cyan"></i></div>
                        <div class="media-body">
                          <h6 class="media-heading"><a href="#" data-id="' . $row['id'] . '" class="view_task">' . $row['name'] . '</a></h6>
	...
	public function pmsverify()
    {
        $receiver_id = $this->aauth->get_user()->id;
		if (!$receiver_id) {
            $receiver_id = $this->CI->session->userdata('id');
        }
		
		$query = $this->db->query("SELECT * FROM geopos_pms WHERE receiver_id = $receiver_id AND date_read IS NULL AND pm_deleted_sender = 0 AND pm_deleted_receiver=0");
		$result = $query->num_rows();
        echo json_encode(array('id' => $receiver_id, 'tcount' => $result));
    }
