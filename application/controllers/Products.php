    ...
    public function product_list()
    {
        ...
        foreach ($list as $prd) {
			...
			if ($cid > 0) {
				...
				{
					$option .= '<a class="btn btn-purple  btn-sm" href="' . base_url() . 'products/edit?id=' . $pid . '" target="_blank"> <span class="fa fa-edit"></span> ' . $this->lang->line('Edit') . '</a> <div class="btn-group">';
				}
				...
				$option = '<a href="#" data-object-id="' . $pid . '" class="btn btn-success  btn-sm  view-object"><span class="fa fa-eye"></span> ' . $this->lang->line('View') . '</a> 
							<div class="btn-group">
                                    <button type="button" class="btn btn-indigo dropdown-toggle btn-sm" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-print"></i>  ' . $this->lang->line('Print') . '</button>
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="' . base_url() . 'products/barcode?id=' . $pid . '" target="_blank"> ' . $this->lang->line('BarCode') . '</a><div class="dropdown-divider"></div> <a class="dropdown-item" href="' . base_url() . 'products/posbarcode?id=' . $pid . '" target="_blank"> ' . $this->lang->line('BarCode') . ' - Compact</a> <div class="dropdown-divider"></div>
				...
				if($this->aauth->premission(24) || $this->aauth->get_user()->roleid == 5 || $this->aauth->get_user()->roleid == 7)
				{
					$option .= '&nbsp;<a href="' . base_url() . 'products/edit?id=' . $pid . '"  class="btn btn-purple btn-sm"><span class="fa fa-edit"></span>' . $this->lang->line('Edit') . '</a>';
				}
				if ($this->aauth->premission(121) || $this->aauth->get_user()->roleid == 7)
				{
					$option .= '<div class="dropdown-divider"></div>&nbsp;<a href="#" data-object-id="' . $pid . '" class="btn btn-danger btn-sm  delete-object"><span class="fa fa-trash"></span>' . $this->lang->line('Delete') . '</a>';
				}
...
//if(!$cat_ware) exit();
    ...
    public function classes_load_list()
    {
        ...
        foreach ($list as $cla) {
			...
			{
				$option .= '<a href="editclass?id=' . $cla->id . '" class="btn btn-primary btn-sm"><span class="fa fa-pencil"></span>  ' . $this->lang->line('Edit') . '</a>';
			}
			if ($this->aauth->premission(121) || $this->aauth->get_user()->roleid == 7)
			{
