...
class Employee extends CI_Controller
{
    ...
    public function save_salary_process(){
        ...
        if ($this->salary_process->save_salary($data)) {
            echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('ADDED') . "<a href='salaries' class='btn btn-grey btn-lg'><span class='icon-eye' aria-hidden='true'></span>  </a>"));
        } else {
	...
	public function fault()
    {
        if ($this->input->post()) {
			...
			{
                echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('ADDED') . "  <a href='fault' class='btn btn-blue btn-lg'><span class='fa fa-plus-circle' aria-hidden='true'></span>  </a> <a href='faults' class='btn btn-grey btn-lg'><span class='fa fa-eye' aria-hidden='true'></span>  </a>"));
            } else {
                echo json_encode(array('status' => 'Error', 'message' => $this->lang->line('ERROR')));
            }
        } else {
	 ...
	 public function flt_list()
    {
        ...
        foreach ($list as $obj) {
            ...
            $row[] = $obj->note;
			$row[] = "<a href='" . base_url("employee/edfault?id=$obj->id") . "' class='btn btn-blue'><i class='fa fa-pencil'></i> " . $this->lang->line('Edit') . "</a> " . '<a href="#" data-object-id="' . $obj->id . '" class="btn btn-danger delete-object"><span class="fa fa-trash"></span></a>';
            $data[] = $row;
        }
