...
class Webupdate extends CI_Controller
{
    ...
    {
        $head['title'] = $this->lang->line('Update');
    ...
    {
		$liccode = $this->db->where("l_stt","act")->get("geopos_license")->row()->l_code;
        $url = file_get_contents(FCPATH . '/version.json');
        ...
        if ($version['build']) {
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, "url=" . urlencode(base_url()) . "&version=" . $version['build'] . "&lic=" . $liccode);
			...
			//print_r($page);
			//echo $url,"  -- ",  $httpcode;
			//echo (" url=" . urlencode(base_url()) . "&version=" . $version['build'] . "&lic=" . $liccode); exit; 
			
            if ((int)$httpcode != 404){
                echo 'License is checked....';
                ...
                if ($httpcode == 503) {
					...
					if (!@mkdir($extractPath, 0777, true)) {
                        ...
                        if (!file_exists($extractPath)) {
                            echo 'Failed to create the update page, please give appropriate write permissions to  ' . FCPATH . 'userfiles' . DIRECTORY_SEPARATOR . 'temp directory  ...';
                            exit;
                        }
                    }
                    $zip = new ZipArchive;
                    ...
                    if ($zip->open($zipFile) != "true") {
                        echo "Error :- Extracting failed! Limited Permissions";
...
";
    ...
    public function install_update()
    {
        ...
        if ($build && $upto) {
            ...
            echo 'Updating files in your system... ' . $build . '';
...
//update files
            $f = 0;
            echo 'File update process started...';
            foreach ($files as $row) {
                ...
                if (copy('userfiles/temp/up' . $build . '_' . $temp_id . '/' . $row['path'] . $row['file'], $row['path'] . $row['file'])) {
                    ...
                    $z = $f + 1;
                    echo 'Files update failed ... File number ' . $z . '';
                }
            }
            ...
            if ($count_f = $f) {
                ...
                echo 'Some Files update failed ... Update Failed!';
...
// Set the url
            ...
            if ($mysqli->close()) {
				...
				$myfile = fopen(FCPATH . '/version.json', "w");
				fwrite($myfile, '{ "version": "0.'.$ver.'", "build": "'.$ver.'"}');
