...
class Search extends CI_Controller
{
    ...
    public function search_invoice()
    {
        ...
        $head['title'] = $this->lang->line('Product Categories');
	...
	public function usermessages()
    {
        ...
        if ($name != NULL) {
            $query = $this->db->query("SELECT r.id,r.username,y.name FROM geopos_users as r inner join geopos_employees as y on r.id = y.id WHERE r.username LIKE '%" . $name . "%' or r.email LIKE '%" . $name . "%' LIMIT 6");
            ...
            echo '<div>';
            foreach ($result as $row) {
                echo '<kbd class="selectuser black" data-username="' . $row['username'] . '" data-userid="' . $row['id'] . '">' .$row['name'].' ('.$row['username'] . ')</kbd> ';
