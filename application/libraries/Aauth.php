...
class Aauth
{
     ...
     * if no license show license page. else set session
     * @return null
     */
	public function chk_cert(){
		$this->aauth_db->where("l_stt","act");
		$r=$this->aauth_db->get("geopos_license")->row();
		if($r){
    ...
    //tested    //updated by dk 15/04/2022 // add license check on session
    ...
    public function is_loggedin()
    {
        ...
        if ($this->CI->session->userdata('loggedin')) {
			if ($this->CI->session->userdata('license_t')) 
				return TRUE;
			else
				return false; // redirect to license scr
    ...
    function hash_password($pass, $userid)
    {
        if ($this->config_vars['use_password_hash']) {
            ...
            //$salt = md5($userid);
			//echo hash($this->config_vars['hash'], $salt . $pass);
    ...
    function applog($input1, $input2='', $typ='', $id_t = 0)
    {
         $this->aauth_db->insert('geopos_log', array('note'=>$input1,'user'=>$input2,'created'=>date('Y-m-d H:i:s'), 'type_log'=>$typ, 'id_c'=>$id_t));
    }
